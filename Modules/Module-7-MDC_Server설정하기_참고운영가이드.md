...
1. 탐지 이벤트 확인
o	포털 접속: https://security.microsoft.com
o	메뉴: 인시던트(Incidents) 또는 경고(Alerts)
o	각 이벤트는 다음 정보를 포함
o	탐지된 위협 이름 
o	탐지된 시간, 디바이스, 사용자
o	관련된 파일, 프로세스, IP, URL 등

2. 인시던트 타임라인 분석
o	인시던트 페이지에서 타임라인을 통해 공격 흐름 확인
o	예: 이메일 수신 → 링크 클릭 → 악성 프로세스 실행 → lateral movement
o	주요 항목:
o	프로세스 트리 (Process Tree)
o	파일 생성/삭제
o	네트워크 연결
o	사용자 로그인/세션 정보
...
o	메뉴: 헌팅 > 고급 헌팅
o	KQL(Kusto Query Language)을 사용하여 관련 이벤트 추적
...
o	프로세스의 행위 분석
o	네트워크 연결지
o	결과는 "자동 조사" 탭에서 확인 가능

5. 샘플 분석 및 제출
o	의심 파일을 수동으로 다운로드하여 분석하거나,
o	Microsoft에 샘플 제출: https://www.microsoft.com/wdsi/filesubmission

6. 외부 위협 인텔리전스 연계
o	VirusTotal, Any.run, Hybrid Analysis 등에서 해시/IP/도메인 분석
o	MDE는 Threat Intelligence와 연동되어 자동으로 평판을 제공

7. 대응 조치 및 예외 처리
o	차단, 격리, 프로세스 종료, 사용자 로그오프 등
o	오탐일 경우 예외 처리 (허용 등록)
분석 시 유용한 KQL 테이블
테이블 이름	설명
...
## 1.8. 에이전트 장애가 발생 시 문제 해결
#### 1.8.1 센서 상태 문제 해결
리눅스 버전의 경우, 아래 트러블 슈팅 가이드 참고 

•	에이전트 상태 문제 조사 - Microsoft Defender for Endpoint | Microsoft Learn

윈도우 버전의 경우, 아래 트러블 슈팅 가이드 참고 

•	엔드포인트용 Microsoft Defender 디바이스 상태 확인 - Microsoft Defender for Endpoint | Microsoft Learn
•	엔드포인트용 Microsoft Defender 비정상 센서 수정 - Microsoft Defender for Endpoint | Microsoft Learn
•	엔드포인트용 Microsoft Defender 비정상 센서 수정 - Microsoft Defender for Endpoint | Microsoft Learn
•	Microsoft Defender 바이러스 백신 검사 문제 해결 - Microsoft Defender for Endpoint | Microsoft Learn
## 1.9. 에이전트 긴급 삭제 혹은 프로세스 중지 방안
#### 1.9.1 에이전트 중지 및 삭제 방법
1. 설정 화면에서 중지
...
o	Microsoft 365 Defender 포털(security.microsoft.com)에 관리자 계정으로 로그인
o	설정 > 엔드포인트 > 디바이스 관리 > 오프보딩 메뉴로 이동
o	오프보딩할 디바이스의 운영 체제(예: 리눅스/Windows Server 등)를 선택
o	오프보딩 패키지를 다운로드

3. 오프보딩 패키지 실행
o	다운로드한 오프보딩 패키지를 오프보딩 대상 디바이스에 복사
o	로컬 스크립트 실행:
오프보딩 패키지 안의 스크립트(.cmd 또는 .ps1)를 관리자 권한으로 실행
o	[참고] 여러 대의 디바이스를 오프보딩할 경우,
그룹 정책(GPO), Microsoft Endpoint Configuration Manager(SCCM), Intune 등 조직의 배포 도구를 활용할 수도 있음

4. 오프보딩 결과 확인
o	서비스 상태 확인:
o	오프보딩이 완료되면 서비스가 중지됨

5. 에이전트 삭제 
o	중지된 이후 제어판에서 직접 제거
o	제어판 > 프로그램 및 기능에서
"Microsoft Defender for Endpoint" 또는 "Windows Defender Advanced Threat Protection"을 찾아 제거를 클릭
o	명령줄(MSI)로 제거 (MMA 기반 환경)
o	Microsoft Monitoring Agent(MMA) 기반 환경에서는
명령 프롬프트(관리자 권한)에서 다음과 같이 입력합니다:
o	%WinDir%\System32\msiexec.exe /x <MOMAgent.msi 경로> /qb
o	또는, 제어판에서 "Microsoft Monitoring Agent" 선택 후 제거

이외에, 중앙 관리(SCCM/Intune)에서 정책 배포로 일괄 오프보딩 및 삭제

#### 1.9.2 에이전트 프로세스 중지 방법
